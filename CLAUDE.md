...
- Command: `/enrich-issue <issue-number>` - Enrich a GitHub issue with codebase triage, pinned code permalinks (to `origin/main` HEAD SHA), and an optional Mermaid sequence diagram illustrating where the flow breaks down. Dispatches a research subagent to map the issue to code locations, then a review subagent to verify quality before posting. Never prescribes solutions. Ends with an optional prompt to refine the issue title/description based on research.
